...
function descargarPDF(boton) {
    ...
    const botones = hoja.querySelectorAll('.imprimir-btn, .pdf-btn');
    botones.forEach(b => b.classList.add('oculto-para-pdf'));
    ...
    html2canvas(hoja).then(canvas => {
      ...
      botones.forEach(b => b.classList.remove('oculto-para-pdf'));
    });
  }
